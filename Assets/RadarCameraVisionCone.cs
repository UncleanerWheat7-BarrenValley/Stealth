...
public class RadarCameraVisionCone : MonoBehaviour
{
    ...
    Mesh GenerateVisionConeMesh()
    {
        ...
        float angleStep = CameraFOV.fovValue / segments;
        float angle = transform.eulerAngles.y - CameraFOV.fovValue / 2f;

        for (int i = 1; i <= segments; i++)
        {
            float angleRad = Mathf.Deg2Rad * angle; // Use the angle directly without adding transform.eulerAngles.y

            Vector3 vertex = (transform.position + Vector3.up * 1.5f) + new Vector3(Mathf.Sin(angleRad), 0, Mathf.Cos(angleRad)) * CameraFOV.depthOfViewValue;
